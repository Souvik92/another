...
class PopularAuthor extends Component{
    ...
    goBefore = () =>{
        ...
        if((this.state.prevPointer-this.props.noOfElements) >= 0){
            this.setState({prevPointer:tempPrevPointer-this.props.noOfElements,nextPointer:tempNextPointer-this.props.noOfElements})
        }else{
    ...
    goAfter =() =>{
        let tempNextPointer = this.state.nextPointer;
        let tempPrevPointer = this.state.prevPointer;
        if((this.state.nextPointer+this.props.noOfElements) <= this.state.totalElements){
            this.setState({prevPointer:tempPrevPointer+this.props.noOfElements,nextPointer:tempNextPointer+this.props.noOfElements})
        }else{
            this.setState({prevPointer:(tempPrevPointer+(this.state.totalElements-this.state.nextPointer)),nextPointer:this.state.totalElements})
            }
    ...
    render(){
        ...
        const authors =this.props.authors.slice(this.state.prevPointer,this.state.nextPointer);
        const authorsHTML = authors.map(el => {
            return (
                <Card myRef={this.inputElement} resizeFunc={()=>this.props.setNoOfElements(this.inputElement.current.clientWidth)}>
                    <Link to={'/author/'+el.name.split(" ").join('').toLowerCase()}>
                        <div >
