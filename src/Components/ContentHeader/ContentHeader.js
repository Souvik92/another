...
const contentHeader = (props) => {
    const elements = props.categories.map(el => {
        return <li key={el["sub-category"]}><span className={getIcons(el["sub-category"])}></span>{el["sub-category"]}</li>
    })
    return (
        <div className={classes.ContentHeader}>
            <div>
                <ul>
                    <li><span className={getIcons(props.categories[0].category)}></span>{props.categories[0].category}</li>
                    {elements}
                </ul>
            </div>
        </div>
